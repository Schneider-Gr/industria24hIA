...
| **2026-07-23** | **Correção de drift:** raiz do repo e `.claude/skills/DESIGN.md` estavam presos na revisão intermediária "Mercado Livre" (amarelo/azul/Archivo) — regredidos acidentalmente por uma reescrita anterior ("do zero") que não herdou a decisão final de 22/07 (tarde). Ambos ressincronizados com esta versão (Vermelho & Roxo / Sora), igual às 5 cópias já corretas (`web/`, `web-venda-futura/`, `web-transportadoras/`, `web-mobile-paineis/`, `web-b2b-auditoria/`) | Andreia reportou "a fonte continua errada" no mockup; investigação encontrou 2 versões divergentes de DESIGN.md coexistindo no repo; Andreia confirmou Sora + Vermelho & Roxo como a correta |
