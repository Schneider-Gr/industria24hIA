...
## Contexto do Produto
- **O que é:** marketplace D2C industrial (Manaus) — comprar direto de quem fabrica, sem intermediário e com grandes descontos; loja grátis, plataforma retém 5% por item.
...
- **Sub-marca Venda Futura (`vf-*`, documentada em 2026-09-11):** `--color-vf-roxo #3D1A5C`, `--color-vf-roxo-claro #6D28D9`, `--color-vf-vermelho #DC2626`, em `globals.css` desde 2026-07-30. Escopo restrito: só componentes de Venda Futura (`MercadoFuturo`, `MercadoFuturoIntro`, faixa da página de produto). Nunca substitui `lm-*` fora desse escopo; o roxo já foi paleta global duas vezes e foi trocado as duas.
...
| 2026-09-19 | Gradiente roxo do bloco de Venda Futura (`MercadoFuturoIntro`) **mantido** como identidade da sub-marca `vf-*`, exceção à regra "sem gradiente roxo-decorativo" | Decisão da dona na crítica `/impeccable critique` da home (19/09). Na mesma rodada a Venda Futura virou um bloco único (topo + datas + FAQ), sem trilho repetido nem cartão ilustrativo |
| 2026-09-19 | `--color-muted` de `#7c7c7c` para `#6b6b6b`; os 16 usos soltos de `text-[#7C7C7C]` passam a usar o token | Audit impeccable: 4,17:1 no branco e 3,93:1 no fundo da página reprovavam no WCAG AA para texto pequeno. `#6b6b6b` dá 5,33:1 e 5,01:1. Mesma troca nos e-mails HTML (`lib/email.ts`, `lib/seller/nova-loja-mensagens.ts`) |
