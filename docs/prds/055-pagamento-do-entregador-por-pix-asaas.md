...
- **Estado atual** (verificado em 25/09/2026):
  ...
  - A corrida calcula o `valor_parceiro` (o `preco_final` menos 10% de comissão), mas **nenhum código paga o entregador**: a tabela `repasses` só aceita `seller` ou `afiliado`, e a chave Pix do parceiro nunca é usada.
...
2. Valor = `valor_parceiro` da corrida (o `preco_final` menos a comissão da plataforma, hoje 10%, migration 0083) *(premissa — confirme ou corrija)*.
...
- **2026-09-25:** Pendentes: valor, momento, taxa, devolução pós-entrega, carência e chave obrigatória (premissas 2 a 7).
