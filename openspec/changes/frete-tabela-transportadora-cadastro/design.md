...
**D9. Categorias por tabela de ligação.** `transportadora_nos (transportadora_id, taxonomia_no_id, primary key (…))`. O contador de produtos usa CTE recursiva sobre `taxonomia_nos.parent_id` a partir dos nós marcados, contando `produtos` da loja. Nó removido: `on delete cascade` apaga a ligação, e um trigger em `taxonomia_nos` marca `transportadoras.revisar_categorias`, que o painel de pendências mostra até o seller salvar as categorias de novo. Sem nó restante, a transportadora volta a "leva tudo" (decisão 6 do PRD), e é por isso que o aviso é obrigatório.

**D10. Grade simples guardada e convertida.** `transportadora_veiculos (transportadora_id, veiculo in ('moto','carro','utilitario'), peso_max, altura_max, largura_max, comprimento_max)` `transportadora_grade_cd (transportadora_id, cd_id, prazo_min, prazo_max, ad_valorem, icms, frete_minimo, taxa_fixa, zonas_nao_atendidas)` e `transportadora_grade_simples (transportadora_id, cd_id, zona, veiculo, preco)`: prazo e parâmetros avançados são da grade do CD, não da célula. `zonas_cep (zona, cep_inicial, cep_final)` é semeada por migration a partir da planilha "Industria24 - Manaus por bairro" (690 faixas de 67 bairros, conferidas no ViaCEP 40/40) mais as faixas de CEP das cidades vizinhas. Salvar a grade chama `substituir_faixas_transportadora` com `p_cd_id`. A grade fica guardada para o seller editar de novo; as faixas são derivadas.
...
**D12. Aviso de tabela global atualizada e encerramento.** Não há tabela de notificações do painel: o padrão do projeto (PRD 047) é e-mail disparado por Vercel Cron diário com idempotência em `alertas_enviados`. Tabela atualizada: `transportadoras.tabela_atualizada_em`, e-mail às lojas que ativaram na confirmação e data exibida no painel. Encerramento: cron diário envia o e-mail em `encerra_em - 7`; a global com `encerra_em <= hoje` deixa de valer nas consultas e o trigger de `loja_transportadoras` recusa ativá-la.
