...
import { criarCupomLoja } from "./actions";
...
const REGRA_VAZIA: Regra = { alvo: "loja", alvo_id: "", tipo: "percentual", valor: "" };

export function NovoCupomLojaForm({ produtos }: { produtos: { id: string; nome: string }[] }) {
  ...
  return (
    ...
    >
      <p className="text-sm text-muted">
        Custeado pela margem do seu produto (reduz o preço antes do repasse) — não é
        subsidiado pela plataforma.
      </p>
      <div className="flex flex-wrap gap-2">
        <input name="codigo" required placeholder="Código (ex.: MINHALOJA10)" className={inputCls} />
      ...
      <div className="space-y-2">
        <p className="text-sm font-semibold">Regras (produto ou loja inteira → desconto)</p>
        {regras.map((r, i) => (
