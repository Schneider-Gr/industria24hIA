...
export default async function ProdutosPage({
  ...
  const [produtosRes, categoriasRes, subcategoriasRes, centrosRes] = await Promise.all([
    supabase
      .from("produtos")
      .select(
        "id, nome, valor, estoque_atual, quantidade_minima, sku, cep_produto, status_produto, created_at, categoria_id, subcategoria_id, permite_afiliacao, porcentagem_afiliado, permite_logistica_afiliado, altura, comprimento, largura, peso, descricao, frete_gratis, perecivel, produto_imagens(url)",
      )
      .eq("loja_id", loja.id)
