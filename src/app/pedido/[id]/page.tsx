    ...
    return (
      <Shell novo={false}>
        ...
        <p className="mt-3 text-center">
          <Link href={`/login?next=/pedido/${id}`} className="text-roxo-800 underline">
            Ir para o login
          </Link>
        </p>
      </Shell>
          ...
          {!pedido.asaas_cobranca_id &&
            (isAsaasConfigured ? (
              <form action={gerarCobranca} className="mt-3 grid grid-cols-2 gap-3">
                ...
                <button
                  type="submit"
                  className="col-span-2 rounded bg-laranja px-5 py-2.5 text-sm font-semibold text-white hover:bg-laranja-escuro"
                >
                  Gerar cobrança
                </button>
              </form>
