...
export async function buscarGaleriasVitrine(
  ...
  const resultado: GaleriaVitrine[] = [];
  ...
  for (const galeria of galerias) {
    if (galeria.tipo === "lancamento" || galeria.tipo === "mais_baratos") {
    ...
    } else if (galeria.tipo === "desconto_progressivo") {
      ...
      const { data: produtos } = await supabase
        ...
        .in("id", ids)
        .gt("valor", 0);
      const imagens = await buscarImagens(supabase, ids);
      ...
      resultado.push({ id: galeria.id, titulo: galeria.titulo, tipo: "desconto_progressivo", produtos: itens });
    } else if (galeria.tipo === "custom") {
      ...
      if (!itens.length) continue;
      resultado.push({ id: galeria.id, titulo: galeria.titulo, tipo: "custom", produtos: itens });
    }
  }
