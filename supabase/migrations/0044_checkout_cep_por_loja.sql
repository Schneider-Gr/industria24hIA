-- 0044: checkout_criar_pedido passa a validar o CEP CONTRA A LOJA do carrinho
-- (0043 deu suporte a faixas_cep por loja) em vez de só a faixa global.
-- Antes, a checagem de CEP rodava ANTES do loop que descobre v_loja — dava
-- pra validar CEP sem nunca cruzar com a loja real. Agora roda depois do
-- loop, casando loja_id = v_loja primeiro e caindo pro fallback global
-- (loja_id is null) se a loja não tiver faixa própria.
...
begin
  ...
  -- CEP validado contra a loja já resolvida: casa faixa da própria loja
  -- primeiro, cai pro fallback global (loja_id is null) se não houver.
  if not v_retirada then
