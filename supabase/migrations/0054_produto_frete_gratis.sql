-- 0054: frete grátis por produto. Seller liga/desliga frete na edição do
-- produto (produtos.frete_gratis). O checkout cobra frete apenas sobre a
-- parte NÃO-gratuita do carrinho (proporcional), e grava valor_frete = 0 na
-- linha do item marcado como frete grátis. Copia integral de
-- checkout_criar_pedido de 0044, mudando só o cálculo de frete.
...
declare
  ...
  v_base_frete numeric(12,2) := 0;  -- soma dos itens que NÃO são frete grátis
...
begin
  ...
  for v_item in select * from jsonb_array_elements(itens) loop
    ...
    select p.id, p.loja_id, p.nome, p.valor, p.estoque_atual, p.quantidade_minima, p.frete_gratis
      into v_prod
    from produtos p
    ...
    if not v_prod.frete_gratis then
      v_base_frete := v_base_frete + (v_preco_unit * v_qtd);
    end if;
  end loop;
  ...
  if not v_retirada then
    v_frete := round(v_base_frete * v_percentual / 100, 2);
  end if;
  ...
  for v_item in select * from jsonb_array_elements(itens) loop
    v_qtd := (v_item->>'quantidade')::int;
    select p.id, p.nome, p.valor, p.frete_gratis into v_prod
    ...
    values (v_pedido, v_prod.id, v_prod.nome, v_qtd,
      ...
      v_vf_id,
      v_retirada,
      case when v_retirada or v_prod.frete_gratis then 0
