...
select to_regclass('public.lotes_consolidacao') as t1, to_regclass('public.lote_pedidos') as t2;
select proname, pronargs from pg_proc join pg_namespace n on n.oid=pronamespace where n.nspname='public' and proname in ('checkout_criar_pedido','criar_lote_consolidacao','despachar_corrida_automatica') order by 1,2;
rollback;
